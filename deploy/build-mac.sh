...
set -euo pipefail
cd "$(dirname "$0")/.."
APP_VERSION="${APP_VERSION:-$(sed -n 's/^version = "\([^\"]*\)"/\1/p' crates/mac-capture/Cargo.toml | head -n 1)}"
./deploy/build-roi.sh release
cargo build --quiet --release -p mac-capture --bin settings --bin mac-capture
...
cp target/release/mac-capture dist/SightRelay.app/Contents/MacOS/mac-capture
...
if [[ -n "${APPLE_SIGN_IDENTITY:-}" ]]; then
  ...
  codesign --force --options runtime --timestamp --sign "$APPLE_SIGN_IDENTITY" dist/SightRelay.app/Contents/MacOS/mac-capture
